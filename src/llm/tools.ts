...
function requireStr(
  ...
  if (typeof val !== 'string') {
    return ok(name, `Error: "${key}" parameter is required and must be a string.`);
  }
  if (opts?.nonEmpty === true && val.trim() === '') {
    return ok(name, `Error: "${key}" parameter is required and must be a non-empty string.`);
  }
  return val;
}
...
export async function executeTool(db: KeeperDB, call: ToolCall): Promise<ToolResult> {
  ...
  switch (name) {
    ...
    case 'search_notes': {
      ...
      if (results.length === 0) return ok(name, `No notes matching "${query}".`);
    ...
    case 'delete_note': {
      const id = requireStr(name, args, 'id');
      if (typeof id !== 'string') return id;
      return confirm(name, `Are you sure you want to delete note "${id}"? This cannot be undone.`);
    }
    ...
    case 'toggle_pin': {
      ...
      return ok(name, `Note ${note.pinned ? 'pinned' : 'unpinned'}.\n${formatNote(note)}`);
    }
    ...
    case 'toggle_archive': {
      ...
      return ok(name, `Note ${note.archived ? 'archived' : 'unarchived'}.\n${formatNote(note)}`);
    }
